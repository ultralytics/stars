...
- GA periods accept zeros (`allow_zero=True`), and an empty GA report also yields zeros; the committed `google_analytics.json` at the time of writing has an all-zero `1d` period next to populated `7d`+ windows, so zeros there are either explicit GA metrics or values synthesized from an empty report — not a merge failure. `analytics.yml`'s header comment describes GA as "page views (30-day window)" — the code fetches `activeUsers`/`sessions`/`eventCount`/`averageSessionDuration` over five windows; trust the code.
