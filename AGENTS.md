...
- `cla.yml` (CLA) — runs on every `pull_request_target` event (opened, reopened, synchronize) and, for `issue_comment: created`, only on PR comments whose body is exactly `recheck` or contains "I have read the CLA Document and I sign the CLA". Uses `ultralytics/actions/cla@main` with `secrets._GITHUB_TOKEN`.
...
**Run flow.** `fetch_stats.py`'s `__main__` block runs the sources in a fixed order, each reading its committed JSON, fetching, merging with `safe_merge`, and writing its file immediately: `fetch_github_stats` → `fetch_pypi_stats` → `fetch_google_analytics_stats` (only if `GA_CREDENTIALS_JSON`) → `fetch_reddit_stats` → `fetch_platform_stats` (only if `PORTAL_API_KEY`) → an inline summary block that builds `data/summary.json` from the in-memory results. Output paths are `Path(__file__).parent / "data/<name>.json"`, so the script is cwd-independent. The GitHub GraphQL stage is the only one that aborts deliberately: `fetch_github_repos` calls `sys.exit` on GraphQL `errors` or a missing organization, and `post_json` exits on any 4xx (including 429) or after three failed attempts, so a GraphQL failure ends the run before anything is written and the workflow step fails (no PR). Contributor lookups and every later source catch their request failures, print `Warning: ...`, and fall back to committed values. Errors outside those handlers still abort mid-run after earlier files were written — for example the `__main__` print of `ga_data["periods"]["1d"]` raises if a fallback `google_analytics.json` lacks a `1d` period.
...
- `fetch_stats.py`
  ...
  - `fetch_pypi_package_stats(package, pepy_api_key)`: pypistats.org `GET /api/packages/{package}/recent` → `last_day`/`last_week`/`last_month`; pepy.tech `GET /api/v2/projects/{package}` with `X-API-Key` → `total_downloads`; 1.0 s sleep per package (the code comment cites pepy's free tier as 10 calls/min). 429/5xx responses are retried (printing `Retry ...` lines) and only raise into the caller's printed warning once all attempts fail; any other non-200 (401/403/404, …) silently leaves those fields at 0.
  ...
  - `parse_abbreviated_number(value)` + `fetch_reddit_stats(subreddit, output)`: shields.io `GET https://img.shields.io/reddit/subreddit-subscribers/{subreddit}.json`, parses `value` like `"4.7k"` (`k`/`m`/`b` suffixes) to an int, merges `subscribers` with `allow_zero=False`; a non-200 other than 429/5xx silently leaves 0 for the merge to replace.
...
- `data/` — the published API. Known consumers: the separate `ultralytics/portal` repo (`apps/portal/lib/community/stats.ts` reads `github.json` `total_*`, `pypi.json` `total_downloads`/`total_last_month`, `google_analytics.json`, and `summary.json` `platform_annotations`/`platform_images`/`platform_models`/`reddit_subscribers`) and the README's public field docs for `github.json` and `pypi.json`. Renaming or dropping a key is a breaking change: keep the old key or update every consumer in the same PR.
  ...
  | ---------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `data/github.json`           | every completed run             | `org`, `total_stars`, `total_forks`, `total_issues`, `total_pull_requests`, `total_contributors`, `public_repos`, `timestamp`, `repos[]` of `{name, stars, forks, issues, pull_requests, contributors}` in fetched-stars order                                                                |
...
- GA periods accept zeros (`allow_zero=True`), and an empty GA report also yields zeros; the committed `google_analytics.json` at the time of writing has an all-zero `1d` period next to populated `7d`+ windows, so zeros there are either explicit GA metrics or values synthesized from an empty report — not a merge failure.
...
There are none: no `tests/`, no pytest or coverage configuration, no `pyproject.toml`. The repository-defined PR workflows are `format.yml` (Ruff, docstring formatting, Prettier, codespell, Lychee link check) and `cla.yml`, plus GitHub's CodeQL default-setup checks; the daily `analytics.yml` run is the only execution of the code. Validate a change by running `python fetch_stats.py` locally with a real `GITHUB_TOKEN` (and the other secrets if your change touches them), or a single fetcher against a scratch `Path` as shown in Commands, reading every `Warning:` line, inspecting `git diff -- data/`, then discarding `data/` before committing. Focused regression tests are out of scope by default (Core Principle 4).
