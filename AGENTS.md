...
- `fetch_stats.py`
  ...
  - `fetch_pypi_package_stats(package, pepy_api_key)`: pypistats.org `GET /api/packages/{package}/recent` → `last_day`/`last_week`/`last_month`; pepy.tech `GET /api/v2/projects/{package}` with `X-API-Key` → `total_downloads`; 1.0 s sleep per package (the code comment cites pepy's free tier as 10 calls/min). 429/5xx responses are retried with `Retry ...` log lines and end in a printed warning; any other non-200 (401/403/404, …) silently leaves those fields at 0.
